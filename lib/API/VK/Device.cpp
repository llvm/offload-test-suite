...
private:
  ...
  //   llvm::StringMap<std::unique_ptr<offloadtest::AccelerationStructure>>
  //   TLASes;
  //   // Vertex/index buffers consumed during AS builds; must outlive
  //   submission. llvm::SmallVector<std::unique_ptr<offloadtest::Buffer>>
  //   ASInputBuffers;
  // };
...
public:
  ...
  createTexture(std::string Name, const TextureCreateDesc &Desc) override {
    ...
    VkImageView View = VK_NULL_HANDLE;
    if (auto Err =
            VK::toError(vkCreateImageView(Device, &ViewCi, nullptr, &View),
    ...
    auto Tex = std::make_unique<VulkanTexture>(
        Device, Image, DeviceMemory, View, Name, Desc, PreferredLayout,
        FullRange, ImageInfo.tiling, MemReqs.size);
...
public:
  ...
  // llvm::Error createDescriptorSets(InvocationState &IS, VkDescriptorPool
  // Pool, llvm::SmallVectorImpl<VkDescriptorSet> &DescriptorSets) {
  //   if (P.Sets.size() == 0)
  //     return llvm::Error::success();
  ...
  //   IS.DescriptorSets.insert(DescriptorSets.begin(),
  //                            VulkanPipeline.SetLayouts.size(),
  //                            VkDescriptorSet());
  //   llvm::outs() << "Num Descriptor sets: " <<
  //   VulkanPipeline.SetLayouts.size()
  //                << "\n";
  //   if (auto Err =
  ...
  //                   &View), "Failed to create image view."))
  //             return Err;
  //           const VkDescriptorImageInfo ImageInfo = {ResRef.Image.Sampler,
  //           View,
  ...
  //           if (auto Err = VK::toError(
  //                   vkCreateBufferView(Device, &ViewCreateInfo, nullptr,
  //                   &View), "Failed to create buffer view."))
  //             return Err;
  ...
  //         CounterWDS.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
  //         CounterWDS.pBufferInfo =
  //         &BufferInfos[IndexOfFirstBufferDataInArray]; llvm::outs() <<
  //         "Updating Counter Descriptor [" << OverallResIdx
  //                      << "] { " << SetIdx << ", " << RIdx << " }\n";
