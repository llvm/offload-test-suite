//===- DX/WSLDevice.cpp - DirectX Device API ---------------------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file is the same as DX/Device.cpp but with a few differences due to
// missing certain APIs only available on Windows:
//   - Microsoft::WRL::ComPtr is used instead of CComPtr from ATL
//   - sys/eventfd.h is used in place of the Windows synchapi.h
//   - The DXCore API is used to acquire devices instead of DXGI
//   - The debug layer is removed since it doesn't appear to work in WSL
//
//===----------------------------------------------------------------------===//
...
#include "DXFeatures.def"
  ...
  llvm::Error createCommandStructures(InvocationState &IS) {
    ...
    if (auto Err =
            HR::toError(Device->CreateCommandList(
                            0, D3D12_COMMAND_LIST_TYPE_DIRECT, IS.Allocator.Get(),
  ...
  llvm::Error createEvent(InvocationState &IS) {
    if (auto Err = HR::toError(Device->CreateFence(0, D3D12_FENCE_FLAG_NONE,
      ...
      return Err;
    IS.EventFD = eventfd(0, EFD_CLOEXEC | EFD_NONBLOCK);
    if (IS.EventFD == -1)
  ...
  llvm::Error waitForSignal(InvocationState &IS) {
    ...
    if (IS.Fence->GetCompletedValue() < CurrentCounter) {
      if (auto Err = HR::toError(
              IS.Fence->SetEventOnCompletion(CurrentCounter, reinterpret_cast<HANDLE>(IS.EventFD)),
              "Failed to register end event."))
        return Err;
      // Wait for signal from eventfd
      pollfd pfd;
      pfd.fd = IS.EventFD;
      pfd.events = POLLIN;
      pfd.revents = 0;
  ...
  llvm::Error executeProgram(llvm::StringRef Program, Pipeline &P) override {
    ...
    if (int Err = close(State.EventFD))
      return llvm::createStringError(
